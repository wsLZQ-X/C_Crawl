...
def main():
    printMetaData()
    workID = UI.askInteger("请输入要爬取的作品 ID")

    workInfo = getWorkInfo(workID)
    log(INFO, f"成功爬到作品 \033[4;32m{workInfo['name']}\033[0m 的信息。")
    log(INFO, f"该作品由 \033[4;94m{workInfo['type']}\033[0m {workInfo['version']} 作者制作")

    compiledWorkURL = getCompiledWorkURL(workInfo)
    log(INFO, f"成功获取作品 \033[4;32m{workInfo['name']}\033[0m 的文件 URL。")

    compiledWork = HTTP.getJSON(compiledWorkURL)
    log(INFO, f"成功得到 \033[4;32m{workInfo['name']}\033[0m 的文件。")
    ...
    try:
        ...
        decompiler.onStart = lambda: log(INFO, f"开始爬取 ，作品名称：\033[4;32m{workInfo['name']}\033[0m。")
        ...
        decompiler.onFinish = lambda: log(INFO, "爬取完成。")
        decompiler.start()
    except:
        showError("无法得到源码！", traceback.format_exc())
...
def saveSourceCode(sourceCode, type):
    ...
    fileTypes = {
        ...
        "KITTEN3": (("Kitten 源码文件", ".bcm"), ("适用于 K3 的 K4 源码文件", ".bcm4")("所有文件", ".*")),
